...
const isColumnNumberOnly = (thPosition, rows) => {
    if (columnMap) return columnMap[thPosition];
    const columns = rows.map((row) => [...row.querySelectorAll("td")][thPosition]);
    columnMap[thPosition] = columns.every(col => !isNaN(Number(col.textContent.trim())));
...
const getHTMLHighlighted = (element, searchValue) => {
    ...
    const walk = (node) => {
        ...
        const html = originalText.replace(regex, `<span data-highlighted="true" class="bg-indigo-600">${match}</span>`);
